function exp1_lasso(n,m,s,maxiter,num_repeats)
...
for repeats = 1:num_repeats
    ...
    for iter = 1:maxiter
        x05 = proj2_L1ball0(v-sum(y05,2),1000);
        index = randperm(m,1);
    ...
    prox = @(x,a,b) x-alpha*project(x/alpha,a,b);
    for iter = 1:maxiter
        x1 = proj2_L1ball0(v-sum(y1,2),1000);
        index = randperm(m,1);
    ...
    prox = @(x,a,b) x-alpha*project(x/alpha,a,b);
    for iter = 1:maxiter
        x15 = proj2_L1ball0(v-sum(y15,2),1000);
        index = randperm(m,1);
    ...
    prox = @(x,a,b) x-alpha*project(x/alpha,a,b);
    for iter = 1:maxiter
        x2 = proj2_L1ball0(v-sum(y2,2),1000);
        index = randperm(m,1);
