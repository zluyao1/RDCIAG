function exp2_unblock_delay2(n,m,s,tau,lambda,maxiter,num_repeats)
...
for repeats = 1:num_repeats
    ...
    x_05 = zeros(n,1);
    y_05 = zeros(n,m,tau+2);  
    x_1 = zeros(n,1);
    y_1 = zeros(n,m,tau+2);
    x_15 = zeros(n,1);
    y_15 = zeros(n,m,tau+2);
    x_2 = zeros(n,1);
    y_2 = zeros(n,m,tau+2); 
    ...
    for iter = 1:maxiter
        ...
        for i = 1:n
            ...
            yever = y_05(:,:,value);            
            x_05(i) = S(-sum(yever(i,:)));
        end
        index = randperm(m,1);
        ynow = y_05(:,:,tau+1);
        ynow(:,index) = prox(ynow(:,index) + alpha*x_05, A(index,:)', b(index));

        y_05(:,:,tau+2) = ynow;
        y_05(:,:,(1:(tau+1))) = y_05(:,:,(2:(tau+2)));
    ...
    for iter = 1:maxiter
        ...
        index = randperm(m,1);
        ynow = y_1(:,:,tau+1);
        ynow(:,index) = prox(ynow(:,index) + alpha*x_1, A(index,:)', b(index));

        y_1(:,:,tau+2) = ynow;
        y_1(:,:,(1:(tau+1))) = y_1(:,:,(2:(tau+2)));
    ...
    for iter = 1:maxiter
        ...
        for i = 1:n
            value = randperm(tau+1,1);
            yever = y_15(:,:,value);            
            x_15(i) = S(-sum(yever(i,:)));
        end
        index = randperm(m,1);
        ynow = y_15(:,:,tau+1);
        ynow(:,index) = prox(ynow(:,index) + alpha*x_15, A(index,:)', b(index));

        y_15(:,:,tau+2) = ynow;
        y_15(:,:,(1:(tau+1))) = y_15(:,:,(2:(tau+2)));
    ...
    for iter = 1:maxiter
        ...
        index = randperm(m,1);
        ynow = y_2(:,:,tau+1);
        ynow(:,index) = prox(ynow(:,index) + alpha*x_2, A(index,:)', b(index));

        y_2(:,:,tau+2) = ynow;
        y_2(:,:,(1:(tau+1))) = y_2(:,:,(2:(tau+2)));
